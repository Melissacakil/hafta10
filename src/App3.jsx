  ...
  switch (aksiyon.type) {
    ...
    case "IS_SIL":
      return state.filter((yapilacakIs) => {
        if (aksiyon.id != yapilacakIs.id) return yapilacakIs;
      });
...
function App() {
  const [yapilacaklar, yapilacaklarVekilFonksiyonu] = useReducer(
    yapilacaklarReducer,
    ilkVeri
  );
  const [kullanici, kullaniciVekilFonksiyonu] = useReducer(
    kullaniciReducer,
    "Hasan Tokatlı"
  );
  ...
  const kullaniciDegistir = (olay) => {
    kullaniciVekilFonksiyonu({
      type: "ISIM_DEGISTIR",
      yeniDeger: olay.target.value,
    });
  };
